...
let CheckOutComponent = class CheckOutComponent {
    ...
    checkOut() {
        this.ordersService.checkOut("bar", this.tableCode).subscribe((res) => console.log(res), (err) => this.errorMessage = err.statusText, () => this.socketService.socket.emit('updateBar'));
        this.ordersService.checkOut("kitchen", this.tableCode).subscribe((res) => console.log(res), (err) => this.errorMessage = err.statusText, () => this.socketService.socket.emit('updateKitchen'));
...
let ToastsComponent = class ToastsComponent {
    ...
    getKitchenMessages() {
        this.ordersService.getOrders(`kitchen`, `?waiter=${this.usersService.user.username}&status=completato`).subscribe((res) => this.messages = res, (err) => console.log(err), () => {
    ...
    getBarMessages() {
        this.ordersService.getOrders(`bar`, `?waiter=${this.usersService.user.username}&status=completato`).subscribe((res) => this.messages = res, (err) => console.log(err), () => {
            for (let i = 0; i < this.messages.length; i++) {
    ...
    pop(order, type) {
        ...
        this.toastr.success('Tavolo : ' + order.tableCode, 'Ordine ' + orderNumber + ' ' + 'da servire', {
            ...
            this.ordersService.updateOrder(type, order).subscribe((res) => { }, (err) => console.log(err), () => {
                if (type == "kitchen") {
                    this.socketService.socket.emit('updateKitchen');
                }
                else if (type == "bar") {
